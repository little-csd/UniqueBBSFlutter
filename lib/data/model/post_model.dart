...
/// TODO: 后续添加数据库层缓存 & 保证即使 _thread 数据有问题也可以正常工作
class PostModel extends ChangeNotifier {
  ...
  PostModel(this._thread) : assert(_thread != null) {
    int tot = getPage(_thread.thread.postCount - 1);
    _fetching = List.filled(tot + 1, false);
  }

  // 包含 threadAuthor 在内的总 post 的个数
  int maxCount() {
    return _thread.thread.postCount;
  }
  ...
  void _fetch(int page) async {
    ...
    Server.instance.postsInThread(_thread.thread.tid, page).then((rsp) {
      if (rsp.success) {
